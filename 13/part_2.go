...
func main() {
	...
	// I am going to filter out blank lines, so allLines will include those blanks but lines won't
	allLines := strings.Split(string(data), "\n")


	// Rather than lines starting empty, it will start with the the divider packets
	lines := []string{ "[[2]]", "[[6]]" }
	// Here I grab the packets that occur every three lines and add them to lines but excluding the blank lines
	for l := 0; l < len(allLines); l+=3 {
		lines = append(lines, allLines[l:l+2]...)
	}

	// This is essentially bubble sort
	for x := 0; x < len(lines); x++ {
		for l := 1; l < len(lines); l++ {
			// If my order checker returns a zero or negative values, we need to make a swap
			if orderCheck(ConvertElement(lines[l-1]), ConvertElement(lines[l])) <= 0 {
				// Nice way to perform an element swap
				lines[l], lines[l-1] = lines[l-1], lines[l]
			}
		}
	}

	// I could have store the indicies of the two divisor packets and multiplied them later but this way is more funky
	out := 1
	for l := range lines {
		if lines[l] == "[[2]]" {
			out *= l + 1
